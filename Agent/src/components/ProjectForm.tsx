...
export const ProjectForm = ({
  ...
  const errors = useMemo(() => {
    ...
    if (!formData.credentials_id.trim()) e.credentials_id = 'Credentials ID is required';
  ...
  const stepErrors = useMemo(() => {
    const e: Record<number, boolean> = {
      1: !!errors.name,
      2: !!errors.git_url || !!errors.credentials_id,
      3: !!errors.sonar_key || !!errors.target_ip || !!errors.target_url
    };
    return e;
  }, [errors, formData]);

  const isStepValid = !stepErrors[step];
    ...
    <div className="bg-white border border-slate-200 rounded-[3rem] shadow-2xl shadow-slate-200/50 overflow-hidden">
      <div className="bg-slate-50/50 border-b border-slate-100 px-10 py-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Step {step} of 3 — System Configuration</span>
        </div>
        <div className="flex gap-1">
          {[1, 2, 3].map((s) => (
            <div 
              key={s} 
              className={`w-8 h-1 rounded-full transition-all duration-500 ${s <= step ? 'bg-blue-600' : 'bg-slate-200'}`}
            />
          ))}
        </div>
      </div>
        ...
        <div className="pt-4 flex items-center gap-4">
          {step > 1 && (
            <button
              ...
              onClick={prevStep}
              disabled={isBlocked}
              className="flex-1 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 py-5 rounded-2xl font-black uppercase tracking-widest text-xs transition-all active:scale-95 disabled:opacity-60"
            >
              Back
            </button>
          )}
          ...
          <button
            type="submit"
            disabled={isBlocked || (step === 3 && !isFormValid) || !isStepValid}
            className={`flex-[2] py-5 rounded-2xl font-black uppercase tracking-widest text-xs transition-all active:scale-95 shadow-2xl flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed ${
              step === 3 ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200' : 'bg-slate-900 hover:bg-slate-800 text-white shadow-slate-200'
            }`}
          >
            {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
            {step < 3 ? 'Continue' : submitLabel}
          </button>
        </div>
      </form>
    </div>
