...
import { Plus, Search, ExternalLink, Activity } from 'lucide-react';
        ...
        <table className="w-full text-left">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Git Source</th>
            ...
            {projects.length === 0 ? (
              ...
              projects.map(project => (
                <tr key={project.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-slate-600 truncate max-w-xs">{project.gitUrl}</div>
                    <div className="text-xs text-slate-400">{project.branch}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${
                        project.lastScanStatus === 'PASSED' ? 'bg-green-500' :
                        project.lastScanStatus === 'FAILED' ? 'bg-red-500' :
                        project.lastScanStatus === 'RUNNING' ? 'bg-blue-500 animate-pulse' : 'bg-slate-300'
                      }`} />
                      <span className="text-sm font-medium text-slate-700">
                        {project.lastScanStatus || 'No scans yet'}
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
