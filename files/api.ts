...
export const api = {
  ...
  projects: {
     ...
     * Requires PATCH /api/v1/projects/:id on the backend.
     * See backend/app/api/projects.py for the required endpoint.
     */
    update: async (id: string, updates: Partial<Omit<Project, 'project_id'>>): Promise<Project> => {
  ...
  scans: {
    ...
    trigger: async (project_id: string, scan_mode: ScanMode, selected_stages?: StageId[]): Promise<Scan> => {
      const response = await apiClient.post('/scans', { project_id, scan_mode, selected_stages });
      return response.data;
    },
