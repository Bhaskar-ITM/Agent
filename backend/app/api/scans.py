...
def _expire_scan_if_timed_out(scan_obj, now=None, timeout_seconds=None):
    ...
    to avoid redundant object creations during bulk scan processing.
    """
...
def list_scans():
    with scans_db_lock:
        # Performance Optimization (Bolt ⚡): Hoist time calculations outside the loop
        # and batch the persist_state call to ensure O(1) disk writes instead of O(N).
        now = datetime.utcnow()
        timeout = settings.SCAN_TIMEOUT
        mutated = False

        for scan_obj in scans_db.values():
            if _expire_scan_if_timed_out(scan_obj, now=now, timeout_seconds=timeout):
                mutated = True
