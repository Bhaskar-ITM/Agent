...
def _expire_scan_if_timed_out(scan_obj, now: datetime, timeout_seconds: int):
...
def list_scans():
    with scans_db_lock:
        # Performance Optimization (Bolt ⚡): Variable hoisting.
        # Pre-calculating 'now' and fetching 'timeout_seconds' once avoids redundant calls and lookups in the loop.
        now = datetime.utcnow()
        timeout_seconds = settings.SCAN_TIMEOUT
        mutated = False
        for scan_obj in scans_db.values():
            if _expire_scan_if_timed_out(scan_obj, now, timeout_seconds):
                mutated = True

        # Performance Optimization (Bolt ⚡): Batch persistence.
        # Previously, persist_state was called inside the loop, causing O(N) disk writes.
        # Now we call it exactly once if any scan state was actually modified.
        if mutated:
...
def get_scan(scan_id: str):
    with scans_db_lock:
        ...
        if _expire_scan_if_timed_out(scan_obj, datetime.utcnow(), settings.SCAN_TIMEOUT):
            persist_state(scans_db, projects_db)
        return _scan_to_response(scan_obj)
